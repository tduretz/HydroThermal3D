...
@parallel function ResidualDiffusionTransient!(F::Data.Array, T::Data.Array, R::Data.Array, qx::Data.Array, qy::Data.Array, qz::Data.Array, _dt::Data.Number,
                            _dx::Data.Number, _dy::Data.Number, _dz::Data.Number)
...
# @parallel function ResidualDiffusion_TestSign!(F::Data.Array, R::Data.Array, qx::Data.Array, qy::Data.Array, qz::Data.Array,
#                                       _dx::Data.Number, _dy::Data.Number, _dz::Data.Number)
#
# 	@all(F) = @all(R) +  _dx*@d_xa(qx) + _dy*@d_ya(qy) + _dz*@d_za(qz)
#
...
@parallel_indices (ix,iy,iz) function SetTemperatureBCs!(Pc_ex::Data.Array, Pbot::Data.Number, Ptop::Data.Number)
...
@parallel_indices (ix,iy,iz) function SetPressureBCs!(Pc_ex::Data.Array, Pbot::Data.Number, Ptop::Data.Number)
	...
	if (ix==1             && iy<=size(Pc_ex,2) && iz<=size(Pc_ex,3)) Pc_ex[1            ,iy,iz] =          Pc_ex[2              ,iy,iz]  end
    if (ix==size(Pc_ex,1) && iy<=size(Pc_ex,2) && iz<=size(Pc_ex,3)) Pc_ex[size(Pc_ex,1),iy,iz] =          Pc_ex[size(Pc_ex,1)-1,iy,iz]  end
    if (ix<=size(Pc_ex,1) && iy==1             && iz<=size(Pc_ex,3)) Pc_ex[ix            ,1,iz] =          Pc_ex[ix              ,2,iz]  end
    if (ix<=size(Pc_ex,1) && iy==size(Pc_ex,2) && iz<=size(Pc_ex,3)) Pc_ex[ix,size(Pc_ex,2),iz] =          Pc_ex[ix,size(Pc_ex,2)-1,iz]  end
    if (ix<=size(Pc_ex,1) && iy<=size(Pc_ex,2) && iz==1            ) Pc_ex[ix,iy,            1] =          Pc_ex[ix,iy,              2]  end
    if (ix<=size(Pc_ex,1) && iy<=size(Pc_ex,2) && iz==size(Pc_ex,3)) Pc_ex[ix,iy,size(Pc_ex,3)] =          Pc_ex[ix,iy,size(Pc_ex,3)-1]  end
...
@parallel_indices (ix,iy,iz) function SetResidualConstrain!(Pc_ex::Data.Array)

	if (ix==1 && iy==1 && iz==1) Pc_ex[ix,iy,iz] = 0 end

    return
end

@parallel_indices (ix,iy,iz) function SetPressureConstrain!(Pc_ex::Data.Array)

	if (ix==2 && iy==2 && iz==2) Pc_ex[ix,iy,iz] = 0 end

    return
end

@parallel_indices (ix,iy,iz) function RemoveMean!(Pc_ex::Data.Array, mean::Data.Number)

	# if (ix>1 && ix<=size(Pc_ex,1)-1 && iy>1 && iy<=size(Pc_ex,2)-1 && iz>1 && iz<=size(Pc_ex,3)-1) Pc_ex[ix,iy,iz] = Pc_ex[ix,iy,iz] - mean end
	if (ix>=1 && ix<=size(Pc_ex,1) && iy>=1 && iy<=size(Pc_ex,2) && iz>=1 && iz<=size(Pc_ex,3)) Pc_ex[ix,iy,iz] = Pc_ex[ix,iy,iz] - mean end
...
@parallel_indices (ix,iy,iz) function KillNorthSouthFluxes!(Ty::Data.Array)

	if (ix>=1 && ix<=size(Ty,1) && iy==1          && iz>=1 && iz<=size(Ty,3)) Ty[ix,iy,iz] = 0.0 end
	if (ix>=1 && ix<=size(Ty,1) && iy==size(Ty,2) && iz>=1 && iz<=size(Ty,3)) Ty[ix,iy,iz] = 0.0 end
...
# @parallel function SetInitialConditions(kfv, Tc_ex, Tbot, Ttop, dT, xce2, yce2, xv2, yv2, Ly, yc2, xmax, ymax)
	...
	@. Tc_ex = -dT/Ly * yce2 + dT
	@printf("dT=%2.2e Ly=%2.2e min(yce)=%2.2e max(yce)=%2.2e\n", dT, Ly, minimum_g(yce2), maximum_g(yce2))
    ...
    @. Tc_ex[ ((xce2-0/2)^2 + (yce2-0.5/2)^2 + (zce2-0/2)^2) < 0.01 ] += 0.1
end
...
# Physics
# Ra       = 80.0 # important to put the digit there - if not, it is defined as an Int64
rho0f    = 1
rho0s    = 1
Cpf      = 1
Cps      = 1
...
ymin     = -0.0;  ymax =  1.0; Ly = ymax - ymin
...
# Initialisation
time_vec = @zeros(nt+1,1,1)
Nu_vec   = @zeros(nt+1,1,1)
...
@time Pc_ex = Array(Pc_ex) # MAKE SURE ACTIVITY IS IN THE CPU:
@time kfv   = Array(kfv)   # Ensure it is temporarily a CPU array
@time SetInitialConditions(kfv, Pc_ex, xce2, yce2, zce2, xv2, yv2, zv2, yc2, Tbot, Ttop, dT, xmax, ymax, zmax, Ly)
...
for it = it1:nt
    ...
    for iter = 0:nitmax
        @parallel cublocks cuthreads SetTemperatureBCs!(Tc_ex, Tbot, Ttop)
        @parallel cublocks cuthreads ComputeFlux!(qx, qy, qz, kx, ky, kz, Tc_ex, _dx, _dy, _dz)
		@parallel cublocks cuthreads ResidualDiffusionTransient!(Ft, Tc_ex, Rt, qx, qy, qz, _dt, _dx, _dy, _dz)      # f      =  RHS_p + d(qx_i)/dx_i
		@parallel cublocks cuthreads UpdateP!(Ft0, Tc_ex, Ft, dampx, dtauT)
	...
	@parallel cublocks cuthreads KillNorthSouthFluxes!(Ty)
    @parallel cublocks cuthreads Compute_Rp!(Rp, ky, Ty, Ra, _dy)                               # RHS_p = Ra* dT/dy

    for iter = 0:nitmax
        @parallel cublocks cuthreads SetPressureBCs!(Pc_ex, Pbot, Ptop)                         # dPdx_i = 0 sur limite
        @parallel cublocks cuthreads ComputeFlux!(qx, qy, qz, kx, ky, kz, Pc_ex, _dx, _dy, _dz) # qx_i   = -k dP/dx_i
        @parallel cublocks cuthreads ResidualDiffusion!(Ft, Rp, qx, qy, qz, _dx, _dy, _dz)      # f      =  RHS_p + d(qx_i)/dx_i
		# mean = mean_g(Ft)
		# @parallel cublocks cuthreads RemoveMean!(Ft, mean)
		# @parallel cublocks cuthreads SetResidualConstrain!(Ft)                                # not working
        @parallel cublocks cuthreads UpdateP!(Ft0, Pc_ex, Ft, dampx, dtauP)
		# @parallel cublocks cuthreads SetPressureConstrain!(Pc_ex)                             # not working
    ...
    time  = time + dt;
    @printf("-> it=%d, time=%.1e, dt=%.1e, \n", it, time, dt);



    #---------------------------------------------------------------------
    if Advection == 1
        @printf("---> Advection solver\n")
        # Advection
        order = 2.0
    ...
    if ( Save==1 && mod(it,nout)==0 )
		...
		h5write(filename, "time", Array(time_vec))
		h5write(filename, "Nu", Array(Nu_vec))
