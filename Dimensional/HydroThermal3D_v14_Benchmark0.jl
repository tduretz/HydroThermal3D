# BENCHMARK 0: no faults/detachment, no topo, run fo 1 My
    ...
    @time for it = it1:nt
        ...
        for iter=1:niter
            ...
            if Hydro
                ...
                @printf("min(k) = %11.4e - max(k) = %11.4e\n", minimum_g(k_ρf)*sc.t, maximum_g(k_ρf)*sc.t )
                # k_ρf .= (1000*1e-17/1e-4) / (sc.ρ*sc.L^2/sc.η )
                @printf("min(k) = %11.4e - max(k) = %11.4e\n", minimum_g(k_ρf)*sc.t, maximum_g(k_ρf)*sc.t )
        ...
        if (Vizu && mod(it, nout) == 0 || it==1)
            ...
            p1 = heatmap(xc*sc.L/1e3, yc*sc.L/1e3, (T1[2:end-1,2:end-1,2]'.*sc.T.-273.15), c=:jet1, aspect_ratio=1, xlim=(0,120), ylim=(-30,5), clim=(minT,maxT), title="T Moho = $(TMoho_str)", titlefont = font(12,"Computer Modern")) 
            # p1 = contour!(xv*sc.L/1e3, yv*sc.L/1e3, phv[:,:,2]', c=:black, aspect_ratio=1, levels=0:3, clim=(minT,maxT)) 

            P1 =  copy(Pc_ex)
            P1[phc_ex.==1.0 .|| phc_ex.==-1.0] .= NaN 
            minP = 1e5/1e6
            maxP = 294
            p2 = heatmap(xc*sc.L/1e3, yc*sc.L/1e3, (P1[2:end-1,2:end-1,2]'.*sc.σ./1e6), c=:jet1, aspect_ratio=1, xlim=(0,120), ylim=(-30,5), clim=(minP,maxP), title="P Moho = $(PMoho_str)", titlefont = font(12,"Computer Modern")) 
            ...
            minV = 1.3e-12*100*year
            maxV = 1.52e-10*100*year
            V1 = sqrt.(VxC[:,:,:].^2 .+ VyC[:,:,:].^2 .+ VzC[:,:,:].^2)
            V1[phc_ex[2:end-1,2:end-1,2:end-1].==1.0 .|| phc_ex[2:end-1,2:end-1,2:end-1].==-1.0] .= NaN 
            p3 = heatmap(xc *sc.L/1e3, yc *sc.L/1e3, (V1[:,:,2]'.*sc.V*100*year), c=:jet1, aspect_ratio=1, xlim=(0,120), ylim=(-30,5), title="|V| [cm/y]"*string(" @ t = ", tMa, " My" ), titlefont = font(12,"Computer Modern") )
            ...
            # p3 = heatmap(xc *sc.L/1e3, yc *sc.L/1e3, (dumc[:,:,2]'), c=:jet1, aspect_ratio=1, xlim=(0,120), ylim=(-30,5) ) #, title="min(k) = $(minimum(dumc[dumc.!=NaN]))  max(k) = $(maximum(dumc[dumc.!=NaN]))"
            ...
            # @parallel InitConductivity!(kx, ky, kz, k_ρf)
            # @parallel ComputeFlux!(qx, qy, qz, kx, ky, kz, Tc_ex, _dx, _dy, _dz)
            # p2 = heatmap(xc *sc.L/1e3, yv*sc.L/1e3, (ky[:,:,2]'.*sc.kt), c=:jet1, aspect_ratio=1, xlim=(0,120), ylim=(-30,5), title="k Moho =$(ky[2,1,2]'.*(sc.kt))"  ) #, title="min(k) = $(minimum(dumc[dumc.!=NaN]))  max(k) = $(maximum(dumc[dumc.!=NaN]))"

            # p3 = heatmap(xc *sc.L/1e3, yv*sc.L/1e3, (qy[:,:,2]'.*(sc.kt*sc.T/sc.L)), c=:jet1, aspect_ratio=1, xlim=(0,120), ylim=(-30,5), title="q Moho = $(qy[2,1,2]'.*(sc.kt*sc.T/sc.L))" ) #, title="min(k) = $(minimum(dumc[dumc.!=NaN]))  max(k) = $(maximum(dumc[dumc.!=NaN]))"


            # p3 = plot!(xce*sc.L/1e3, y_topo*sc.L/1e3, c=:white)

            # p1 = heatmap(xc*sc.L/1e3, yc*sc.L/1e3, phc[:,:,2]', c=:jet1, aspect_ratio=1, xlim=(0,120), ylim=(-30,5)) 
