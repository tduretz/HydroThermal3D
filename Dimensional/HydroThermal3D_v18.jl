...
@views function HydroThermal3D()
    ...
    @time for it = it1:nt
        ...
        if (Vizu && mod(it, nout) == 0 || it==1)
            ...
            T1 = copy(Tc_ex)
            T1[phc_ex.==1.0 .|| phc_ex.==-1.0] .= NaN  # Mask out the air
            minT = 20.
            maxT = 620.
            ...
            fig = Figure()
            #-----------#
            ax = Axis(fig[1,1], aspect=DataAspect(), title="T Moho = $(TMoho_str) (C)")
            p1 = heatmap!(ax, xc*sc.L/1e3, yc*sc.L/1e3, (T1[2:end-1,2:end-1,2].*sc.T.-273.15), colormap=:jet1)#, aspect_ratio=1, xlim=(xmin*sc.L/1e3, xmax*sc.L/1e3), ylim=(-30,5), clim=(minT,maxT), title="T Moho = $(TMoho_str)", titlefont = font(12,"Computer Modern")) 
            Colorbar(fig[1, 2], p1)

            P1 = copy(Pc_ex)
            P1[phc_ex.==1.0 .|| phc_ex.==-1.0] .= NaN # Mask out the air
            minP = 1e5/1e6
            maxP = 294

            #-----------#
            ax = Axis(fig[2,1], aspect=DataAspect(), title="P Moho = $(PMoho_str) (MPa)")
            p2 = heatmap!(ax, xc*sc.L/1e3, yc*sc.L/1e3, (P1[2:end-1,2:end-1,2].*sc.σ./1e6), colormap=:jet1)# aspect_ratio=1, xlim=(xmin*sc.L/1e3, xmax*sc.L/1e3), ylim=(-30,5), clim=(minP,maxP), title="P Moho = $(PMoho_str)", titlefont = font(12,"Computer Modern")) 
            Colorbar(fig[2, 2], p2)

            #-----------#
            ax = Axis(fig[1,3], aspect=DataAspect(), title="Phases"*string(" @ t = ", tMa, " (My)" ))
