...
@views function HydroThermal3D()
    ...
    @time for it = it1:nt
        ...
        if (Vizu && mod(it, nout) == 0 || it==1)
            ...
            p3 = heatmap(xv *sc.L/1e3, yv *sc.L/1e3, (phv[:,:,2]'), c=:jet1, aspect_ratio=1, xlim=(xmin*sc.L/1e3, xmax*sc.L/1e3), ylim=(-30,5), title="Phases"*string(" @ t = ", tMa, " My" ), titlefont = font(12,"Computer Modern") )
