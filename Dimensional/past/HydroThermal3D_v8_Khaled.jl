...
@parallel_indices (i,j,k) function UpdateHydroConductivity(kv, kfv, ρfv, Tce, Pce, yv2, phv, scale_σ, scale_T, scale_L, scale_t, scale_ρ)
    if i<=size(kfv, 1) && j<=size(kfv, 2) && k<=size(kfv, 3) 
        ...
        #     # @show Tv, Pv/1e6
        # end
        kv[i,j,k]  = kF(yv2[i,j,k], δ) / scale_L^2
        if phv[i,j,k] == 1.0
            kv[i,j,k]  = 0.
        end
...
@views function HydroThermal3D()
    ...
    tolP     = 1e-13
    tetP     = 1/4/3
    dtauP    = tetP/6.1*min(dx,dy,dz)^2/sc.t * 3e34
    Pdamp    = 0.09
    ...
    for it = it1:nt
        ...
        for iter = 1:nitmax
            @parallel SetTemperatureBCs!(Tc_ex, qyS, _dy, 1.0/sc.kt, Ttop,  geometry.y_plateau, geometry.a2, geometry.b2, dTdy, dx, dy, sticky_air)
            @parallel ComputeFlux!(qx, qy, qz, kx, ky, kz, Tc_ex, _dx, _dy, _dz)
            @parallel ResidualTemperature!(Ft, Tc_ex, Tc0, Pc_ex, phc, qx, qy, qz, _dt, _dx, _dy, _dz, ϕ, Qt, transient, sc.T, sc.σ, sc.C, sc.ρ)
        ...
        @printf(">>>> Darcy solver\n");
        @parallel ResetA!(Ft, Ft0)
        @parallel UpdateHydroConductivity(kv, kfv, ρfv, Tc_ex, Pc_ex, yv2, phv, sc.σ, sc.T, sc.L, sc.t, sc.ρ)
        ...
        for iter = 1:nitmax
            ...
            # @parallel SmoothConductivityV2C( kc, kfv )
            # @parallel SmoothConductivityC2V( kfv, kc )
            @parallel SetPressureBCs!(Pc_ex, Pbot, Ptop, geometry.y_plateau, geometry.a2, geometry.b2, 1000/sc.ρ, g, dx, dy, sticky_air)
        ...
        if (Vizu == 1)
            ...
            p1 = heatmap(xce*sc.L/1e3, yce*sc.L/1e3, (Tc_ex[:,:,2]'.*sc.T.-273.15), c=cgrad(:hot, rev=true), aspect_ratio=1, clims=(0, 700)) 
            # p1 = heatmap(xce*sc.L/1e3, yce*sc.L/1e3, (Pc_ex[:,:,2]'.*sc.σ./1e6), c=:jet1, aspect_ratio=1) 
            p2 = heatmap(xc*sc.L/1e3, yv*sc.L/1e3, (Vy[:,:,2]'.*sc.V*100*year), c=:jet1, aspect_ratio=1, title="Vy [cm/y]"*string(" @ t = ", tMa, " My" ), clims=(-27, 23)) 
            ...
            # p2 = heatmap(xv*sc.L/1e3, yv*sc.L/1e3, (kv[:,:,2]'.*sc.L^2), c=:jet1, aspect_ratio=1) 


            # p1 = heatmap(xv*sc.L/1e3, yv*sc.L/1e3, log10.(kfv[:,:,2]'.*sc.t), c=:jet1, aspect_ratio=1) 
            # p1 = heatmap(xv*sc.L/1e3, yv*sc.L/1e3, (ρfv[:,:,2]'.*sc.ρ), c=:jet1, aspect_ratio=1) 
            ...
            p = plot!(xlim=(0,120), ylim=(-30,5))
            p3 = plot(xc*sc.L/1e3, Vy[:,izero-1,2])
