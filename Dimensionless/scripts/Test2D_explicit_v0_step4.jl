...
function main()
    ...
    for it = 1:nt
        ...
        if mod(it, nout)==0 || it==1
            p1 = heatmap(xce, yce, T', title=time, aspect_ratio=1, xlims=(-0.5,0.5)) 
            p2 = heatmap(xce, yce, P', aspect_ratio=1, xlims=(-0.5,0.5), title="Initial pressure", xlabel="x []", ylabel="y []") 
            # p = heatmap(xce, yce, T', aspect_ratio=1, xlims=(-0.5,0.5), title="Initial temperature", xlabel="x []", ylabel="y []") 

            # p1 = heatmap(xv, yc, Vx', aspect_ratio=1, ylims=(-0.5,0.5))
            # p2 = heatmap(xc, yv, Vy', aspect_ratio=1, ylims=(-0.5,0.5)) 
            display(plot(p1,p2))
            sleep(0.2)
        end
    end
